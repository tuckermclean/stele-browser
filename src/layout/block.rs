...
///
/// packet/acid1-coherence: `box_sizing` is set to `ContentBox` for floated
/// boxes only -- see `float_box_sizing`'s own doc comment for why this is
/// scoped to floats rather than applied unconditionally (the CSS-correct
/// fix), and why that scoping is deliberate, not an oversight.
...
///
/// packet/acid1-coherence found this via `fixtures/css1-float-5526c.html`
/// (the W3C CSS1 Acid1 test, every element in it sets an explicit `width`
/// AND non-zero `padding`/`border`, which is exactly the combination where
/// `BorderBox` vs `ContentBox` produces different pixels): `dd{width:34em;
/// padding:1em;border:1em}` rendered ~10% narrower inside than it should
/// (its OWN border-box pinned to the declared 34em instead of growing to
/// 34em-content + 2em padding + 2em border), which starved its floated
/// `<li>`/`<blockquote>`/`<h1>` children of the row width they needed to
/// wrap the way the reference rendering does.
///
