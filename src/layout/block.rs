...
fn translate_any<'a>(
    ...
    match &node.content {
        ...
        BoxContent::Container | BoxContent::TableCell { .. } => {
            ...
            // SPIKE spike/taffy-grid: unconditional, exactly like
            // `apply_flex` right above -- `apply_grid` itself is a no-op
            // whenever `cs.grid_template_columns`/`rows` are both empty
            // (every non-grid container, unchanged behavior), see its own
            // doc comment.
...
/// AND `display: grid` (SPIKE spike/taffy-grid: a grid container needs the
/// exact same "every child is its own item" treatment a flex container
/// already gets — an un-grouped `Text`/inline run inside a grid container
/// would otherwise get folded into one taffy leaf per module docs' default
...
fn translate_container_children<'a>(
    ...
    if matches!(node.style.display, Display::Flex | Display::Grid) {
        for child in &node.children {
            ...
            // packet). SPIKE spike/taffy-grid: CSS Grid's own "Grid Items"
            // section makes the identical carve-out for a whitespace-only
            // text child, so this applies unchanged to a grid container's
...
/// SPIKE spike/taffy-grid: wires `cs.grid_template_columns`/`.
/// grid_template_rows` onto taffy's `Style.grid_template_columns`/`.
/// grid_template_rows` (the `grid` cargo feature re-enabled in `Cargo.toml`
/// for this spike). A no-op for any container that never declared either
/// property (`cs.grid_template_columns`/`rows` both start `Vec::new()` --
/// `ComputedStyle::default`) -- taffy's own `Style::DEFAULT` already leaves
/// both empty too, so calling this unconditionally for every container
/// (`translate_any`'s `Container` arm, right after `apply_flex`) changes
/// nothing for a non-grid container, exactly like `apply_flex` itself is
/// already a no-op for one that never set `flex-*`. `gap` needs no
/// separate wiring here -- `apply_flex` (called right before this) already
/// sets `style.gap` unconditionally, and taffy's grid algorithm reads the
/// SAME `Style.gap` field a flex container's does.
...
/// spike); `String` (the same type taffy's own `Style::grid_template_
...
fn map_display(d: Display) -> TDisplay {
    match d {
        ...
        // SPIKE spike/taffy-grid: hands off to taffy's own grid algorithm
        // (the `grid` cargo feature re-enabled in `Cargo.toml` for this
        // spike) -- see `Display::Grid`'s own doc comment (`style/
        // computed.rs`) for the full contract.
