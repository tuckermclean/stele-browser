...
pub fn layout_tree<M: Metrics>(root: &LayoutNode, viewport: Size, metrics: &M) -> Vec<Fragment> {
    ...
    let built = translate_any(root, &mut taffy);
...
fn translate_any<'a>(node: &'a LayoutNode, taffy: &mut TaffyTree<Vec<InlineRun>>) -> Built<'a> {
    match &node.content {
        ...
        BoxContent::Container => {
            ...
            let children = translate_container_children(node, taffy);
...
/// here (documented M2 scope limitation) — a `Replaced` grandchild inside a
/// nested inline `Container` is rare and does not currently get its own box;
/// a *direct* child of the block container still gets one via the grouping
/// loop in `translate_container_children`.
fn flatten_inline(node: &LayoutNode, out: &mut Vec<InlineRun>) {
    match &node.content {
...
fn translate_container_children<'a>(node: &'a LayoutNode, taffy: &mut TaffyTree<Vec<InlineRun>>) -> Vec<Built<'a>> {
    let mut out = Vec::new();
    if node.style.display == Display::Flex {
        for child in &node.children {
            out.push(translate_any(child, taffy));
        }
        return out;
    }
    ...
    while i < node.children.len() {
        if is_inline_ish(&node.children[i]) {
            ...
            while j < node.children.len() && is_inline_ish(&node.children[j]) {
                flatten_inline(&node.children[j], &mut runs);
                j += 1;
            }
            ...
            out.push(translate_any(&node.children[i], taffy));
            i += 1;
        }
    }
    out
