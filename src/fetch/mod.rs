...
mod dispatch_tests {
    ...
    #[test]
    fn fetch_routes_about_scheme_to_the_about_fetcher() {
        // `about:` must dispatch to `about::fetch` -- proving the new match
        // arm, mirroring `fetch_rejects_unknown_scheme` below for what it
        // does NOT match. `about::fetch` is total (never `Err`), so success
