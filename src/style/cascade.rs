...
fn resolve(d: &Declarations, parent: Option<&ComputedStyle>, env: &Env) -> ComputedStyle {
    ...
    ComputedStyle {
        ...
        // SPIKE spike/taffy-grid: non-inherited ("own") resolution, same
        // treatment `flex_direction`/etc. above already get -- real CSS
        // `grid-template-columns`/`rows` aren't inherited either.
...
/// SPIKE spike/taffy-grid: resolve a raw `grid-template-columns`/`rows`
/// value (`None` if the declaration was never set) into `ComputedStyle`'s
/// own `Vec<GridTemplateComponent>` -- `None` resolves to an empty `Vec`,
