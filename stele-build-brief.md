...
Build: `RUSTFLAGS="-Zunstable-options -Cpanic=immediate-abort" cargo build --release --target targets/i486-monolith-linux-musl.json -Zbuild-std=std,panic_abort`
(packet size-squeeze-floppy: -Cpanic=immediate-abort drops std's formatted-
...
1.44MB floppy ceiling; `-C` is a rustc flag, not a cargo one, so it rides
RUSTFLAGS scoped to this one build invocation, never Cargo.toml's
[profile.release] since that profile is shared with the host unit-test
build, which needs normal panic/assert + #[should_panic] behavior — so
host test builds are unaffected)
