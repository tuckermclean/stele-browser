...
#   3  tools are absent and were not installed here — either the bootstrap was
#      not requested (see STELE_BOOTSTRAP_AUDIT_TOOLS below), or it was and
#      could not run (no cargo, or crates.io unreachable). Expected offline;
#      the caller degrades A6 to PENDING.
...
# Installing is OPT-IN (D75). The tools are missing from the pinned image on
# every run, so installing unconditionally would add a multi-minute uncached
# `cargo install` to every accept.sh — buying nothing, because the audit itself
# is PENDING until M6 either way. Default is therefore a cheap presence probe;
# the M6 audit sets STELE_BOOTSTRAP_AUDIT_TOOLS=1 to actually bootstrap. The
# capability is what DCX-151 blocked, and the capability is what this preserves.
BOOTSTRAP="${STELE_BOOTSTRAP_AUDIT_TOOLS:-0}"
...
if [ "$BOOTSTRAP" != "1" ]; then
  echo "audit tooling: bootstrap not requested — set STELE_BOOTSTRAP_AUDIT_TOOLS=1 to install" >&2
  exit 3
fi
