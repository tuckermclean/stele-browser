...
## 2026-08-20 -- Terminus font replaces font8x8 (#93) [back-filled]

- **Back-filling this entry now (packet/attestation-modal, 2026-08-21):** `DECISIONS.md` picked up D65/D66
  for this era of work, but `JOURNAL.md` had no entry for the Terminus font packet's own landing -- found
  during this session's ground-truthing. Recorded here, out of strict chronological order at the bottom of
  the file per AGENTS.md's own "keep this document alive" spirit, rather than left permanently missing.
- **Landed:** the crude 8x8 `font8x8` bitmap font (ASCII+Latin-1) is replaced everywhere by an embedded
  191-glyph Terminus 4.49.1 subset (OFL-1.1: ASCII `0x20-0x7E` + Latin-1 `0xA0-0xFF`, 5 pixel buckets
  6x12/8x16/10x20/12x24/16x32 x normal+bold), generated from the upstream BDFs by
  `tools/gen-terminus-glyphs.py` into a committed `src/text/terminus_glyphs.rs`. `TerminusFont` snaps a
  requested `size_px` to the nearest of the 5 buckets (discrete hinted bitmaps, not continuous scaling);
  `font-weight: bold` is now wired to Terminus's real bold weight (previously cascaded but never rendered).
  16px (the default) metrics are byte-identical to the old `vga_8x16`, so non-text layout at the default size
  is unchanged; ~25 text goldens were re-blessed for the sub-14px/bold fidelity changes.
- **Licensing:** `third_party/terminus-font/OFL.TXT` (verbatim) + `NOTICE.md` attribute the subset under
  OFL-1.1 without misusing the reserved font name "Terminus Font".
- **Size:** i486 binary **1,377,436 bytes = 93% of the 1.44 MB floppy** (97,124 B headroom).
- See DECISIONS D66 for the full options/choice/why.

## 2026-08-21 -- Attestation page: `about:attestations` (packet/attestation-modal)

- **Landed** a new `about:` scheme handler (`src/fetch/about.rs`, one match arm in `fetch::fetch`, infallible
  over all input) serving a self-rendered `about:attestations` page: Stele's own short GPL-3.0 notice, the
  full Cargo runtime-dependency/license roster (generated by `tools/gen-attestations.py` from a real
  i486-target-filtered `cargo metadata` graph into `src/fetch/attestations_data.rs` -- 24 deps, 19
  content-hash-deduped license blocks, ~28.8 KB embedded), and Terminus's complete OFL-1.1 text
  (`include_str!`). Fixed a real, previously-latent bug along the way: `resolve_url` (`main.rs`) didn't pass
  `about:` URLs through, so no CLI entry point (`--dump-text`/`--dump-png`/`--render-fb`/`--x11`) could reach
  the new handler until the one-line fix landed. Reachable from the `--x11` chrome via a new affordance rect
  (`ChromeLayout::attest`, between `address` and `throbber`); the click-to-navigate wiring is manual-verify
  only (no X11 test harness here), its geometry/paint are CI-golden-tested. New goldens
  `goldens/attestations.png`/`.txt`, gated by `accept.sh` A5x (PNG; cross-build stable, same as A5v/A5w) and
  rendered into the CI gallery alongside a `--dump-text` dump for reviewer legibility.
- **Also fixed, adjacent and cheap in this same PR (found during ground-truthing, not this packet's original
  scope):** `REPORT.md`'s Dependencies & licenses table and closing License section were stale, still
  crediting the deleted `font8x8_basic`/`font8x8_ext_latin` (removed by the Terminus packet, #93) -- refreshed
  to the real Terminus/OFL-1.1 + generated-roster ground truth; `third_party/terminus-font/NOTICE.md`'s "lands
  via the forthcoming attestation modal" phrasing updated now that it has landed; and (above) this ledger's
  own missing Terminus entry back-filled.
- **Charter ruling:** a new URL scheme + a self-served page (zero new elements/properties -- headings, `<p>`,
  `<ul>`/`<li>`, all already-adopted dialect), the same "transport surface, not dialect surface" category as
  `data:`/`https` -- no C2 amendment. See DECISIONS D67 for the full options/choice/why, including the
  short-GPL-notice and taffy-license-pin judgment calls flagged for the operator.
